...
def renormalize(W, H):
	""" Renormalizes factors of W to have constant energy.
	Updates passed parameters.
	"""
	L, N, K = W.shape
	factor_norms = np.zeros(K)

	for k in range(K):
		factor_norms[k] = norm(W[:,:,k])
		W[:,:,k] = np.divide(W[:,:,k],factor_norms[k]+EPSILON)

	H.assign(np.multiply(H.shift(0).T,factor_norms+EPSILON).T)
